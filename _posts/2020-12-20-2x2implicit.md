...
date: 2020-12-20 
description: Implicit and explicit solutions for all cases. Proofs now included!
published: true
nav: true
---
Solutions
...
It turns out when $$\operatorname{tr}(A)=0$$, the solution to (\ref{sys}) can always be expressed implicitly as the quadratic form (\ref{quadform}). Conversely, any quadratic form of a $$2\times2$$ symmetric matrix $$S$$, $$x^TSx=C$$, will be the implicit solution to (\ref{sys}) when $$A=QS$$ for any skew-symmetric matrix $$Q$$ (Proof below).
...
$$\begin{equation} \tag{13} \label{usub}
...
$$\begin{equation} \tag{14} \label{chainrule}
...
$$\begin{equation} \tag{15} \label{implicit u}
...
Using (\ref{chainrule}), we divide $$\frac{dy}{dt}$$ by $$\frac{dx}{dt}$$ to get $$\frac{dy}{dx}$$.
...
$$\begin{equation} \tag{16} \label{complexusub}
\begin{bmatrix}x\\y\end{bmatrix}=
\begin{bmatrix}1&0\\\alpha&\beta\end{bmatrix}
...
$$\begin{equation} \tag{17} \label{nonzerorealmultiplied}
...
$$\begin{equation} \tag{18} \label{polar}
